...
如果問 Ruby 裡哪一個套件是最重要最具有影響力的，我想肯定就是 RubyGems 吧。
有了套件管理系統，才有方便的散佈方式，也才有方便使用各種套件的方法。而在 
scripting 的世界裡，方便幾乎可說是最重要的一件事。不是說沒有 RubyGems 就
不會有今天的 Ruby，如果真的沒有，那一定會有誰發明了什麼東西站在這個位子上。
但一個堪用的套件管理系統，跟一個很方便很強大的套件管理系統，就完全不同了。

有些 Ruby 核心開發者，甚至[希望能把標準程式庫][stdlibgem]，改放到 RubyGems 
裡面。這不代表這些標準程式庫不受青睞了，因為它們仍然會預先被安裝到標準 Ruby 
發行版裡面。重點在於，於是這些標準程式庫就可以不受限於 Ruby 本身的發行狀況，
需要修正什麼錯誤時，可以自行推出修正檔，而非請大家等候下一版本的 Ruby。
事實上，rake 在 Ruby 1.9 中，正是這個狀況，一安裝好 Ruby 就可以注意到 rake 
已經被預先安裝好了，使用者也可以自行透過 `gem` 移除或是升級。之後只是希望
更多的標準程式庫可以用這種方式呈現而已。

當然其實也是有反對意見，因為這很可能會使得版本變得更過於混亂，比方說如果
net/http 在 Ruby 2.0 變成了一個標準 gem，那開發者是否可以假設一台跑著 
Ruby 2.0 的電腦，上面有沒有安裝 net/http？會不會被使用者移除掉了？會不會
被使用者升級到一個版本差異過大的版本？

不過個人覺得這個問題是可以被接受的。如果版本差異真的這麼重要，那麼或許
